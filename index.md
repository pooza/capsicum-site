...
**v1.35.0**（2026-06-09） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済み

v1.35 は、**投稿フォームのワードサジェスト**を主役とするリリースです。アニメ実況などで IME の変換候補に出てこない専門用語（必殺技名・キャラクター名など）を、ひらがなの読みから検索して投稿フォームに挿入できます。辞書を用意した[モロヘイヤ](https://github.com/pooza/mulukhiya-toot-proxy)導入サーバーで利用でき、実況の即時性を損なわずに正しい表記を入力できる独自機能です。絵文字（カスタム / Unicode）と劇中ワードを同じシートから開ける共有ピッカーに、通常の投稿フォームと簡易投稿バーの両方を集約しました。
...
これからは、Misskey チャット / ページの続編機能と投げ銭の macOS / Windows 横展開（v1.36）に取り組み、[ナウプレ取得](/now-playing)も Apple Music（v1.37）・Spotify（v1.38）へと源を広げていきます。
