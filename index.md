...
**v1.44.0**（2026-07-05） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです

Mastodon 4.6 の新機能への対応が目玉のリリースです。

- コレクション（FEP-7aa9）に対応しました。最大25アカウントをまとめて紹介・共有でき、自分が載せられたコレクションの確認や掲載の辞退もできます。
- プロフィールのタブ表示設定（メディア・返信付きメディア・注目の投稿）に追従するようにしました。
- アイコン／ヘッダー画像の代替テキスト、ミュートの有効期限、注目の絵文字など、各種メタデータへの追従を強化しました。
- 等々
