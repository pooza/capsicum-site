...
**v1.56.0**（2026-08-13） Android（Google Play）・Linux AppImage で公開済みです。iOS App Store・macOS Mac App Store・Windows Microsoft Store は審査中で、通り次第そちらでも入手できるようになります。

エラーの伝わり方を改善し、起動まわり・下書きまわりの取りこぼしをまとめて直したリリースです。

- サーバーが投稿などを断ったとき、その理由が伝わるようにしました。（下書きが上限に達している、既にリアクション済み、など）
- 投稿フォームを 2 枚開いたときに、投稿済みの本文が下書きとして復活する問題を修正しました。
- 画面ロック中に起動したとき、ログアウトされたように見えることがある問題を修正しました。
- ブロックした相手の投稿が、次の起動時に一覧へ戻ってくることがある問題を修正しました。
- 起動直後に一覧を下まで送ったとき、読み込んだ続きが捨てられることがある問題を修正しました。
- Linux で、起動用に保存した一覧を同じ端末の他のユーザーが読める状態になっていた問題を修正しました。
