...
**v1.31.1**（2026-06-04） Android（Google Play）製品版・Linux AppImage・Windows MSIX 自己署名直配は公開済み、iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p) は審査提出済み

v1.31 の主役は、**タイムライン上の各投稿へのタッチ操作**です。誤タップ防止のため長押しメニューに格納していたお気に入り・ブースト・リアクション・返信・引用といった操作を、設定から**端末ごと・操作別にオプトインで**タイムライン上のボタンとしても有効化できるようになりました（既定はすべて OFF。従来の長押しメニューも併存します）。

v1.31.1 は、リリース直後に検出したアクションメニューのクラッシュを修正したホットフィックスです。
