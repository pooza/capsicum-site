...
**v1.40.0**（2026-06-24） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです

v1.40 は、Windows 版の仕上げを主役とするリリースです。Windows のプッシュ通知をネイティブに本配線し（中継サーバーからの Windows 通知サービス〔WNS〕経由で、アプリを完全に終了している間も通知が届きます）、あわせてデスクトップの常駐（ウィンドウを閉じてもトレイに常駐し通知を受け続ける）・ログイン時の自動起動に対応しました（いずれも Windows で先行、macOS / Linux は後続）。
