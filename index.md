...
**v1.24.4**（2026-05-11） Android 製品版・iOS App Store・macOS Mac App Store すべて公開済み、Linux 版は AppImage を hotfix 4 まで配布中

v1.24 では、デスクトップ展開の第3段階として **Linux 版を初公開** しました。AppImage を [GitHub Releases](https://github.com/pooza/capsicum/releases) から即時ダウンロードできるほか、Flathub への配信も並行して進めています。**macOS 版も同時に Mac App Store で公開**し、Apple Music などの「共有」メニューから capsicum へ投稿できる Share Extension を追加しました。Universal Purchase により iOS / iPadOS と同一の App として配信しています。

v1.24.1〜v1.24.4 は、Linux AppImage 版で日本語入力が効かない不具合の hotfix シリーズです。v1.24.3 で真因（AppImage 内 bundled GLib と host libibus の symbol mismatch）を特定し libibus を明示同梱、v1.24.4 で fcitx5 / uim の immodule も同梱しました。Android / iOS / macOS は v1.24.0 のまま継続しています。

## デスクトップ版について

iOS / Android のモバイル版に加え、macOS / Linux / Windows のデスクトップ版にも段階的に展開しています。動機は、iOS 版を Mac 上で実況用途に使って得た手応えです。

- **macOS** — v1.21 でネイティブビルドの土台を完成させ、v1.23 で通知・抽象化層、v1.24 で実機検証と Share Extension まで完了し、Mac App Store で配信中です。Universal Purchase により iOS / iPadOS と同一の App として提供しています
- **Linux** — v1.24 で初公開しました。AppImage を [GitHub Releases](https://github.com/pooza/capsicum/releases) から即時配布しているほか、[Flathub](https://flathub.org/) への配信も準備中です。日本語入力は `ibus-mozc` のみ動作確認済みで、`fcitx5` / `uim` 経路は **未検証 (best-effort)** です
