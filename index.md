...
**v1.29.0**（2026-05-28） Android 製品版・Linux AppImage・Windows MSIX 自己署名直配・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p) は公開済み、iOS App Store と macOS Mac App Store は審査提出済み

v1.29 の主目的機能は、**[プリセットサーバー](/preset-servers)向けのお知らせ通知配信**です。サーバー管理者が発した「お知らせ」を、[モロヘイヤ](https://github.com/pooza/mulukhiya-toot-proxy) + capsicum-relay 経由で APNs / FCM 経路に流し、プリセットサーバーのユーザーに push 通知としてお届けします（デフォルト ON、設定からオフ可能）。

## ロードマップ

- [**v1.30**](https://github.com/pooza/capsicum/milestone/38) — **Linux / Windows での動画再生対応**（`video_player` → `media_kit` 移行）が主役。あわせて Linux AppImage 配布の体験補強（アプリ一覧への統合インストールスクリプト・新版通知）、ドラッグ＆ドロップ添付・メディアダウンロード等のデスクトップ UX 改善
