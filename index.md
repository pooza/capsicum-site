...
**v1.37.0**（2026-06-17） iOS App Store・Android（Google Play）・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです

v1.37 は、[ナウプレ取得](/now-playing)を iPhone / iPad / Mac の **Apple Music** へ広げたリリースです。これまで Linux / Windows のデスクトップだけだった「投稿フォームの ♪ ボタンで再生中の曲を取得」が、Apple Music の再生中の曲にも対応しました。あわせて、URL を持たない源から取得した曲でも[プリセットサーバー](/preset-servers)側で共有可能な URL を補完する仕組みを実装し、「削除して再編集」での添付・閲覧注意・メンションの引き継ぎも改善しています。
