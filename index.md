...
こまやかな改善を数多く詰め込んだ UX バッチです。

- 投稿フォームでは、ひらがなで打った読みから劇中ワード候補をその場に表示するインライン補完します。
- 別のアカウントを選んでブースト / リノートできるようにしました。
- デスクトップ版（macOS / Windows）では、メディアビューアの画像を Finder / ファイラーへドラッグ＆ドロップで取り出せるようにしました。
- 公開範囲アイコンや相対日時にツールチップを表示
- Windows のプッシュ通知まわりを堅牢化
- 起動の体感速度の改善
- Mastodon 4.6 の新しい通知種別への追従
- 等々
