...
**v1.62.0**（2026-09-01） 全プラットフォームで公開済みです。

Android で画面の下端がナビゲーションバーに隠れる問題を、まとめて解消したリリースです。

- 画面の最下部がナビゲーションバーに隠れる問題を直しました。スレッド画面でのご報告でしたが、原因は画面固有のものではなく、40 以上の画面が同じ形をしていました。
- スレッド画面の ↑ / ↓ ボタンが本文に重なり、スクロールしても逃がせない問題を直しました。ボタンのぶんだけスクロールできる範囲を広げています。
- メニュー（ドロワー）の末尾にあるサーバー情報とバージョン番号が隠れる問題を直しました。
- ホームの「通知」タブで、最後の通知がナビゲーションバーに隠れる問題を直しました。通知を単独の画面で開いたときは直っていましたが、タブとして表示したときだけ残っていました。
