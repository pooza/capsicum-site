...
**v1.57.0**（2026-08-16） すべてのプラットフォームで公開済みです。

お知らせの届き方と、デスクトップ版のメニューまわりを広げたリリースです。

- アプリを終了していても[お知らせ](/push-notification)が届くようになりました。（macOS・Windows）
- デスクトップ版のメニューバーから辿れる操作を増やしました。（ドライブ画面・添付メディア・スレッドで選択中の投稿など）
- サーバーに保存した下書きを、投稿フォームからも選べるようになりました。（Misskey）
- 画像に重ねた文字やスタンプを回転できるようになりました。
- Misskey の実績解除が、実績名つきの通知として出るようになりました。
- 画面ロックなどで認証情報が一時的に読めなかったとき、自動で復帰しないことがある問題を修正しました。
