...
**v1.59.0**（2026-08-22） すべてのプラットフォームで公開済みです。

書きかけの投稿と設定まわりを作り直したリリースです。投稿の自動保存が「いつ保存されたか」まで見えるようになり、端末を移すときにアカウントの一覧ごと引き継げるようになりました。

- 投稿の自動保存を作り直しました。入力が止まってから数秒で保存し、フォームを閉じたときにも保存します。入力欄の下に最後に保存した時刻が出るので、保存されているかどうかが目で見て分かります。本文だけでなく、CW・公開範囲・閲覧注意・ローカルのみも保存され、アカウントごとに分かれて記録されます。
- 設定のバックアップをスマートフォンでも使えるようにしました。書き出しは共有シート、読み込みはファイル選択から行えます。
- 設定のバックアップにアカウントの一覧を含めるようになりました。新しい端末では、サーバー選択画面から直接読み込めます。読み込んだアカウントは「未接続」として並び、ログインし直すと使えるようになります（ログイン情報そのものは含みません）。
- プロフィール画面からも通報できるようにしました。
- メニューから選んだ操作が、送信されないまま何も出ずに消えることがあった問題を修正しました。
- Misskey で画像の説明（ALT）を空にできない問題を修正しました。
