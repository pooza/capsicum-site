...
**v1.32.0**（2026-06-04） Android（Google Play）製品版・Linux AppImage・Windows MSIX 自己署名直配・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p) は公開済み、iOS App Store・macOS Mac App Store は審査提出済み

v1.32 は、デスクトップ / パッケージング周りの内部リファクタを集約した**大きな新機能のないメンテナンスリリース**です。コードの重複解消・プラットフォーム判定の共通化・観測性の改善が中心で、見た目はほぼ変わりません。ユーザー体験に関わる変更としては、通知画面の投稿に対するタッチ操作をタイムラインの投稿タイルと同じ挙動に統一（ブースト済み投稿の取り消しに対応）し、メディア保存失敗時のエラー計装を追加しました。
