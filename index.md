...
- 表示・動作・履歴などの設定をファイルに書き出し、別の端末で読み込めるようにしました。（アカウント・ログイン情報は含みません）
- 番組表に次回の放送日時を表示し、並び順もサーバーの指定どおりに固定しました。
- 投稿フォームを離れても書きかけが残り、次に開いたときに復元されるようにしました。
- Misskey Play の出目が Misskey Web と一致するようにしました。
- 「削除してタグづけ」した投稿が、リロードしなくてもすぐ表示されるようにしました。
- 絵文字ピッカーの入力候補から「非推奨」カテゴリを外しました。（パレットや「最近使った」に入っているものは今までどおり使えます）
- オフラインから復帰したときに、自動で再接続するようにしました。
- macOS でログインした直後に赤い画面が出ることがある問題を修正しました。
- Windows の通知まわりの不具合（起動中の二重表示・別々の通知が 1 つにまとめられる など）を修正しました。
- 端末に残った古い通知の購読が原因で、同じ通知が繰り返し届くことがある問題を修正しました。
