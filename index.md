...
**v1.53.0**（2026-08-01） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです。

起動の速さとキーボード操作を中心に、使い勝手をまとめて改善したリリースです。

- アプリを起動したとき、前回のタイムラインと読んでいた位置をすぐに表示するようにしました。読み込みを待たずに続きから読めます。
- タイムラインとスレッドをキーボードだけでたどれるようにしました（デスクトップ版）。
- デスクトップ版のメニューバーが、投稿画面や簡易投稿バーの状況に合わせて切り替わるようになりました。
- サーバーからのお知らせが自動で最新の内容に更新されるようになりました。
- 投稿の削除・「削除してタグづけ」・ブロックなどの結果が、表示中のタイムラインにすぐ反映されるようになりました。
- 投稿本文に使うフォントを等幅フォントに変えられるようにしました。
- Misskey のチャンネルへリノートできるようにしました。
- Linux 版でも投げ銭（サポート）の案内を表示するようにしました。
- 送信ボタンを続けて押しても投稿が二重に送られないようにするなど、細かな不具合を修正しました。
