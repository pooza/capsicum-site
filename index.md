...
**v1.60.0**（2026-08-23） すべてのプラットフォームで公開済みです。

画像の説明（ALT）まわりと、書きかけの投稿・設定の取りこぼしを詰めたリリースです。

- Mastodon で、投稿済みの画像の説明（ALT）を編集できるようにしました。これまでは一度投稿すると直せませんでした。サーバー側の対応が必要な機能のため、対応していないサーバーではメニューに出ません（プリセットサーバーは対応作業中です）。
- 画像の説明（ALT）の文字数を、入力中に数えて表示するようにしました。これまでは上限を超えたまま投稿すると「投稿に失敗しました」としか出ず、理由が分かりませんでした。上限はサーバーによって違うので、接続先に合わせて表示します。すでに書いてある説明を勝手に切り詰めることはしません。
- 投稿フォームのフォント設定が保存されないことがある問題を直しました。
- Mastodon のエラーを日本語で表示するようにしました。これまでは `Validation failed: Text character limit of 3000 exceeded` のような英語がそのまま出ていました（網羅はしていないため、載せていない文面は英語のまま出ます）。
- 投稿フォームの自動保存が、復元の「取消」と重なったときに壊れる問題を修正しました。取り消したはずの本文が残る・その画面の自動保存が二度と効かない・書き直した本文が消える、といった症状が出ていました。
- Windows で通知が 1 通も表示されないことがある問題を修正しました。
- 設定のバックアップで、取り込めなかった理由が表示されない問題を修正しました。
- 設定の保存に失敗しても「読み込みました」と表示され、再起動すると元に戻っていた問題を修正しました。
