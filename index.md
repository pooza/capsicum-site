...
**v1.64.0**（2026-09-12） 全プラットフォームで公開済みです。

ログインまわりの不具合を中心に、お寄せいただいた報告と実機での検証で見つかった問題を直したリリースです。

- **Android**: ブラウザで「承認」を押しても capsicum に戻らないことがある問題を直しました。ブラウザを操作しているあいだに Android がアプリを一時停止させていたのが原因です。ログインが終わるまでのあいだだけ通知が表示されます。
- **Linux**: キーリング（gnome-keyring など）が応答しないと、起動時に真っ黒なウインドウのまま固まる問題を直しました。応答しないときは画面で案内し、キーリングが戻れば起動し直さずに復帰します。あわせて、キーリングの解錠に失敗したときにログイン情報を消してしまう問題も直しました。
- 「削除して再編集」で、元の投稿の設定を引き継ぐようになりました。返信先・「ローカルのみ」（Misskey）・言語・投票・引用許可（Mastodon）です。これまでは返信を再編集すると単独の投稿になったり、「ローカルのみ」が黙って外れて連合に流れたりしていました。
- Misskey の返信にも、タイムラインで「返信」の印が付くようになりました。
- 一覧の続きの読み込みに失敗したとき、「続きを読み込めませんでした」と再試行ボタンを出すようにしました。
- 操作は成功しているのに「失敗しました」と表示されることがある問題など、細かな不具合をあわせて直しています。
