...
- **劇中ワードサジェスト**（v1.35〜） — アニメ実況などで IME の変換候補に出てこない専門用語（必殺技名・キャラクター名など）を、ひらがなの読みから検索して投稿フォームに挿入できます。v1.41 では、ひらがなを打つだけで候補がその場に表示されるインライン補完も追加しました。辞書を用意した[モロヘイヤ](https://github.com/pooza/mulukhiya-toot-proxy)導入サーバーで利用できる、実況の即時性を支える独自機能です
- **[ナウプレ](/now-playing)** — 聴いている曲を `#nowplaying` 付きの投稿としてワンアクションで作成。音楽アプリの「共有」から作るほか、投稿フォームの ♪ ボタンから再生中の曲を直接取得できます（デスクトップの Linux / Windows は v1.33〜、iPhone / iPad / Mac の Apple Music は v1.37〜）
- 複数サーバー・アカウントの切り替え
...
**v1.41.0**（2026-06-26） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです

v1.41 は、こまやかな改善を数多く詰め込んだ UX バッチです。投稿フォームでは、ひらがなで打った読みから劇中ワード候補をその場に表示するインライン補完を追加し、別のアカウントを選んでブースト / リノートできるようにしました。デスクトップでは、メディアビューアの画像を Finder / ファイラーへドラッグ＆ドロップで取り出せるようにし（macOS / Windows）、公開範囲アイコンや相対日時にツールチップを表示するなど操作性を高めています。あわせて Windows のプッシュ通知まわりを堅牢化し、起動の体感速度の改善や Mastodon 4.6 の新しい通知種別への追従なども進めました。
