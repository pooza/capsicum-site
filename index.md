...
**v1.34.0**（2026-06-07） Android（Google Play）・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage は公開済み、iOS App Store・macOS Mac App Store は審査提出済み

v1.34 は、**デスクトップ 3 OS（macOS / Linux / Windows）の通知本配線**を主役とするリリースです。アプリの起動中に、Mastodon の `user` ストリーム / Misskey の `main` チャンネルへ常時 WebSocket 接続し、届いた通知を OS のローカル通知（macOS の通知センター / Linux の libnotify / Windows のトースト）に流します。あわせて macOS では APNs 経由のネイティブ push 通知の土台も実装しました。これでモバイル（iOS / Android）に続き、デスクトップでもタイムラインを開きっぱなしにせず通知を受け取れます。
