...
**v1.48.0**（2026-07-16） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです。

日々の使い勝手を、皆さまからのご要望を集めて幅広く磨いたリリースです。

- 投稿内の URL を、対応する専用アプリ（YouTube・Apple Music・Spotify・Amazon など）で開けるようにしました。
- Misskey のタイムラインでも、リンク先の概要（プレビューカード）を表示するようにしました。
- 投稿内のハッシュタグをコピーできるようにしました。
- 添付画像の編集（ALT・トリミング・回転・文字入れ）を一つのメニューにまとめました。
- MFM の装飾・アニメーション表現に対応しました。
