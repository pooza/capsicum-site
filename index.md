...
**v1.46.0**（2026-07-11） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです。

サーバーの素性やメタデータの取得・鮮度を中心に磨いたリリースです。

- サーバーソフトのバージョンや利用できる機能を、アプリを再起動しなくても最新の状態に更新するようにしました。
- サーバー情報画面に、プレ公開日と設立日を沿革として表示するようにしました。
- グループ（コミュニティ）アカウントの投稿をタイムライン上で見分けられるようにし、そのプロフィールへ辿れるようにしました。
- サーバー種別の判定を強化し、PieFed など一部のサーバーの情報も取得できるようにしました。
- 接続できなくなったアカウントを消さずにオフライン表示で残し、必要なら削除もできるようにしました。
...
- [**v1.47**](https://github.com/pooza/capsicum/milestone/57) — デスクトップ版のメニューバー対応、ログインまわりの改善など
