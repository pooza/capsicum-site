...
**v1.58.0**（2026-08-18） すべてのプラットフォームで公開済みです。

いただいた不具合報告に急ぎで対応したリリースです。

- 朝いちばんの起動でカスタム絵文字が読めず、しばらく投稿できなくなる問題を修正しました。端末の回線が起き上がりきる前にアプリが動き出すと、ごく短い接続の失敗でアカウントがまとめてオフラインに落ちていました。接続が戻れば自動で復帰します。
- リアクションが送信されないまま、成功も失敗も出ずに消えることがあった問題を修正しました。
- ドライブで複数選択中に「上の階層へ」が使えてしまう問題を修正しました。
- macOS で通知の二重表示を抑える仕組みが、長く起動しているときに効かなくなることがある問題を修正しました。
- 番組表で、放送日を持たない枠に「毎日」と表示するのをやめました。
