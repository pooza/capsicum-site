...
**v1.38.0**（2026-06-19） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです

v1.38 は、[ナウプレ取得](/now-playing)の仕上げとアカウントまわりの信頼性向上を中心としたリリースです。ナウプレの URL 優先プロバイダを選べるようにし、共有から作るナウプレも投稿フォームから作るものと同じ整形に統一しました。あわせて、起動時の一時的な通信エラーで複数アカウントが一斉にログアウト表示になる問題を修正し、投稿した内容がタイムラインにすぐ反映されるよう改善しています。
