...
**v1.47.0**（2026-07-13） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです。

デスクトップ版の操作性と、日々の使い勝手を中心に磨いたリリースです。

- デスクトップ版（Windows / Linux）に、ウィンドウ内のメニューバーを追加しました。
- 投稿やアカウントの URL をタップしたとき、外部ブラウザではなくアプリ内のスレッド・プロフィールで開くようにしました。
- Mastodon のお知らせにリアクションできるようにしました。
- Misskey のサーバー側での下書き保存に対応しました。
- ログイン時、サーバー側のレート制限によって正しい情報でもログインできなくなることがある問題を緩和しました。
...
- [**v1.48**](https://github.com/pooza/capsicum/milestone/58) — ユーザーの皆さまからのご要望を集めた、使い勝手の改善のまとめ
