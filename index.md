...
- 設定のバックアップ
  - 表示・動作・履歴などの設定をファイルに書き出し、別の端末で読み込めます。（アカウント・ログイン情報は含みません）
- [プッシュ通知](/push-notification)
...
**v1.54.0**（2026-08-07） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです。

添付画像へのスタンプを目玉に、プッシュ通知と起動まわりの不具合をまとめて直したリリースです。

- 添付した画像に、カスタム絵文字をスタンプとして重ねられるようにしました。位置とサイズを調整して合成できます。
- Windows 版で、アプリを起動している間だけ同じ通知が二重に表示される問題を修正しました。
- 端末を再起動したあとに古い通知の購読が残り、同じ通知が繰り返し届くことがある問題を修正しました。
- デスクトップ版のメニューバーの画面連動を、残りの画面にも広げました。Windows / Linux では編集メニューも使えます。
- アプリの起動直後、前回のタイムラインを表示している間に行った操作が取りこぼされないようにしました。
- オフラインで起動したときにサーバー選択画面へ飛んでしまう問題を修正しました。
- ログインに成功した直後にアプリが落ちることがある問題を修正しました。
- ブースト・リアクションのポップアップが、別の投稿の内容を表示することがある問題を修正しました。
- リアクションの絵文字ピッカーの高さを調整できるようにしました。
- 通信の遅いサーバーで、翻訳や検索がアプリ側の待ち時間切れで打ち切られてしまわないよう、上限を見直しました。
