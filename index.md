...
v1.18 で、Mastodon / Misskey サーバーが発行する Web Push を APNs / FCM に変換する専用の中継サーバーを経由するプッシュ通知方式に移行しました。従来のバックグラウンドポーリング方式と異なり、iOS でも実用的に通知が届きます。
...
**v1.19.1**（2026-04-22）Google Play 公開済み・App Store 審査中

## ロードマップ

- [**v1.20**](https://github.com/pooza/capsicum/milestone/24) — プッシュ通知の完成（transient リトライ・ペイロード復号・登録失敗の可視化・通知タップ経路の整備等）＋ 中継サーバーの可視性強化
- [**v1.21**](https://github.com/pooza/capsicum/milestone/25) — macOS ネイティブビルド化（デスクトップ対応 第1段階）
...
v1.18〜v1.19 でプッシュ通知リレーを導入・ホットフィックスを経て実運用に乗せました。v1.20 では、実運用で明らかになった残課題の完全消化と中継サーバー側の観測性強化を行い、プッシュ通知を「完成」と呼べる状態に仕上げます。v1.21 以降はデスクトップ環境への段階的な展開が主軸です。iOS 版を Mac 上で実況用途に使い手応えを得たことが動機で、macOS ネイティブ化からスタートし、通知モデルの再設計を経て、Linux / Windows に広げていきます。
