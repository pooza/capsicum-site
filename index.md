...
**v1.42.1**（2026-06-30） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです

ストリーミングの信頼性向上とデスクトップ体験の強化が中心のリリースです。

- タイムラインのライブ更新で、通知なく切れた接続（無音切断）を検知して自動で再接続するようにしました。接続状態の表示もより正直・厳密に。
- ライブ更新への復帰時に、切れていた間の投稿を取りこぼす問題を根治しました（特に Misskey）。
- デスクトップ版（macOS）にグローバルメニューバーを追加しました。
- 常駐／ログイン時の自動起動を macOS（メニューバー）・Linux（トレイ）にも広げました。
- デスクトップ版で、投稿を右クリックすると長押しと同じアクションメニューを開けるようにしました。
- 等々
