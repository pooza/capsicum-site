...
**v1.66.0**（2026-09-22） 全プラットフォームで公開済みです（Windows は v1.66.1）。

返信の宛先を Web UI と同じにしたリリースです。あわせて、前回持ち越した不具合をまとめて直しました。

この版から、動作対象が **iOS 15 以降 / macOS 12 以降**になります。Apple の開発環境が、これより古い OS 向けのアプリを作れなくなったためです。

- **返信の宛先に、返信先の投稿に含まれるメンション全員が入るようになりました。** これまでは返信先の投稿者だけが入り、会話に加わっていた他の人には届いていませんでした。宛先は本文に入るので、送る前に消すこともできます。
- **Mastodon**: 投稿の文字数の上限を、サーバーの設定から読むようになりました。上限を広げているサーバーでも 500 文字として表示していた問題が直ります。
- **Misskey**: 引っ越したアカウントのプロフィールから、引っ越し先へ辿れるようになりました。
- **Windows**: 長い投稿への返信など、内容が大きい通知が届かなかった問題を直しました（v1.66.1）。上限を超える通知は「(アカウント) に通知があります」と表示されます。
- **Linux**: パスワード保管庫（キーリング）に保存できずにログインが失敗したとき、その旨が表示されるようになりました。
- 設定のバックアップで絵文字パレットが保存されていなかった問題や、本文の文字数を実際より少なく数えることがある問題もあわせて直しています。
