...
**v1.28.0**（2026-05-26） Android 製品版・Linux AppImage・Windows MSIX 自己署名直配・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p) は公開済み、iOS App Store と macOS Mac App Store は審査提出済み

v1.28 の主目的機能は、**Misskey のグループチャット対応**（チャットルーム機能）です。あわせて Misskey の **ページ機能の読み取り対応**（[Misskey Pages](https://misskey-hub.net/docs/learn/features/pages/)、AiScript なしの静的ブロック限定 v1 ビューア）と、Misskey ドライブでの **複数ファイルの一括フォルダ移動** も同梱しました。
