...
**v1.43.0**（2026-07-02） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです

Windows 版の投げ銭（サポーター）対応が目玉のリリースです。

- Windows 版（Microsoft Store）で投げ銭（サポーター）に対応しました。iOS / Android / macOS と同じくストア経由で購入できます。
- Windows の公式配布を Microsoft Store 単独に整理しました。
- Android のログイン方式を刷新し、一部端末で「ログイン画面から進まない」問題を解消しました。あわせて外部ブラウザのパスワードマネージャー拡張が使えます。
- ログインの安全性を強化しました（OAuth に PKCE と state 照合を導入）。
- 等々
