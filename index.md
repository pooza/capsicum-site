...
**v1.39.0**（2026-06-21） Android（Google Play）・iOS App Store・macOS Mac App Store・[Windows Microsoft Store](https://apps.microsoft.com/detail/9np2gr7m2w6p)・Linux AppImage の全プラットフォームで公開済みです

v1.39 は、ユーザーからの要望を中心とした UX 改善バッチです。スレッド表示の改善（対象のリプライ位置へのアンカー・先頭／末尾へのジャンプ）、各種表示オプション（起動時に既読位置を復元するか最新先頭で開くかの選択・投稿元サーバー帯の非表示・サムネイル高さの統一・ストリーミング接続状態の常時インジケータ）、入力まわりの利便性（Cmd / Ctrl + Enter での送信・MFM 装飾の挿入メニュー・挿入ピッカーの高さ記憶）などを束ねて改善しています。
