...
## Misskey で公開範囲に「指名」が選べません

v1.63 から、Misskey では投稿画面の公開範囲から「指名」を外しています。

Misskey の「指名」は、**宛先のユーザーを明示的に指定して初めて相手に届く**仕組みです（本文に `@ユーザー名` と書いても宛先にはなりません。この点は Mastodon の「ダイレクト」と異なります）。capsicum には宛先を選ぶ画面がないため、「指名」を選んで新しく投稿すると、エラーも出ないまま**誰にも届かない**投稿ができていました。「選べるのに届かない」より「選べない」ほうが安全と判断しています。

次の点はこれまでどおりです。

- **届いた「指名」投稿は読めます。** 受信側の表示は変えていません。
- **「指名」の投稿への返信もできます。** 返信のときは返信先の相手をサーバーが自動的に宛先へ足すため、正しく届きます。
- **Mastodon の「ダイレクト」は影響ありません。** Mastodon は本文のメンションがそのまま宛先になる仕組みのため、これまでどおり選べます。

Misskey で 1 対 1 のやり取りをする場合は、「メッセージ」（Misskey のチャット）をご利用ください。

なお、下書きの復元や再編集で「指名」が入ったままの投稿は送信できません。その場合は画面に理由を表示しますので、公開範囲を選び直してください。
