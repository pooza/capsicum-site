...
すべてのプラットフォームで、モバイル版と同じ Flutter コードベースから配信しています。UI はウィンドウ幅に応じて自動でレスポンシブに切り替わり、Finder / ファイラー起点のメディア添付やメディアビューアからファイラーへの画像ドラッグ＆ドロップ取り出し（macOS / Windows、v1.41〜）、再生中の曲を投稿フォームに挿入する[ナウプレ取得](/now-playing)（Linux / Windows は v1.33〜、macOS の Apple Music は v1.37〜）、共有メニューからの[ナウプレ共有](/now-playing)（macOS）、アプリ起動中の[通知](/push-notification)を OS のローカル通知に表示する仕組み（3 OS 共通、macOS はネイティブ push にも対応、v1.34〜）といったネイティブな体験を取り入れています。
...
| Windows | Microsoft Store | **v1.34 で起動中の通知に対応（共通経路 [#569](https://github.com/pooza/capsicum/issues/569)）、v1.40 でネイティブ WNS に対応（[#474](https://github.com/pooza/capsicum/issues/474)、アプリ終了中も配信）** | **v1.33 で対応（SMTC [#484](https://github.com/pooza/capsicum/issues/484)）** | 対応外 | ○ (v1.30 で [media_kit 移行 #492](https://github.com/pooza/capsicum/issues/492)) |
...
- **v1.42〜** — デスクトップのグローバルメニューバー対応（ドロワーの項目をネイティブのメニューバーにミラー、macOS 先行 [#712](https://github.com/pooza/capsicum/issues/712) → Windows / Linux の in-window メニューバー [#713](https://github.com/pooza/capsicum/issues/713) は後続）と、常駐／ログイン時自動起動の macOS（メニューバー）・Linux（トレイ）展開（[#757](https://github.com/pooza/capsicum/issues/757)）
- **v1.43** — Windows 版の投げ銭（サポーター）購入導線（Microsoft Store IAP、[#599](https://github.com/pooza/capsicum/issues/599)）と、自己署名 MSIX 直配の段階的廃止 → Store 単独化（[#760](https://github.com/pooza/capsicum/issues/760)）

直近の達成: v1.27 で Windows の Microsoft Store 公開（Partner Center 手動 publish ルート、2026-05-20 審査通過）、v1.28 で Misskey のグループチャット対応、v1.29 で[プリセットサーバー](/preset-servers)向けのお知らせ通知配信。Linux 配布は Flathub 提出を断念し AppImage 単独運用に確定（2026-05-29、[#604](https://github.com/pooza/capsicum/issues/604)）。v1.30 で Linux / Windows の動画再生に対応（[media_kit 移行 #492](https://github.com/pooza/capsicum/issues/492)）し、あわせて Linux AppImage のデスクトップ統合インストール（[#640](https://github.com/pooza/capsicum/issues/640)）・直配チャネルの新版通知（[#641](https://github.com/pooza/capsicum/issues/641)）・ドラッグ＆ドロップ添付などのデスクトップ UX を強化。v1.32 でデスクトップ / パッケージング周りの構造整理（リファクタ集約）を完了。**v1.33 で[ナウプレ取得](/now-playing)（再生中の曲を投稿フォームに挿入）に対応しました — Linux は MPRIS（[#466](https://github.com/pooza/capsicum/issues/466)）、Windows は SMTC（[#484](https://github.com/pooza/capsicum/issues/484)）。**整形はクライアント側で行うため、モロヘイヤ未導入のサーバーでも動作します。**v1.34 でデスクトップ 3 OS の[通知](/push-notification)本配線（アプリ起動中の WebSocket streaming → OS のローカル通知、macOS はネイティブ APNs も）を完了**し、**v1.36 で投げ銭（サポーター）を macOS の Mac App Store に横展開**、**v1.37 で macOS の[ナウプレ取得](/now-playing)を Apple Music に対応**し、**v1.38 でナウプレの仕上げ（URL 優先プロバイダの選択・共有ナウプレの整形統一）を完了**し、**v1.40 で Windows 版の仕上げ — アプリを完全に終了している間でも通知を届けるネイティブ WNS [プッシュ通知](/push-notification)（[#474](https://github.com/pooza/capsicum/issues/474)）と、ウィンドウを閉じてもトレイに常駐して通知を受け続ける常駐（[#752](https://github.com/pooza/capsicum/issues/752)）・ログイン時の自動起動（[#751](https://github.com/pooza/capsicum/issues/751)）に対応**しました（常駐とログイン時起動は Windows で先行、macOS / Linux は [#757](https://github.com/pooza/capsicum/issues/757) で後続）。**v1.41 ではデスクトップの操作性を細かく改善 — メディアビューアの画像を Finder / ファイラーへドラッグ＆ドロップで取り出す機能（macOS / Windows、[#645](https://github.com/pooza/capsicum/issues/645)。Linux は [#776](https://github.com/pooza/capsicum/issues/776) で後続）、公開範囲アイコン・相対日時のツールチップ（[#753](https://github.com/pooza/capsicum/issues/753) / [#754](https://github.com/pooza/capsicum/issues/754)）、Windows プッシュ通知の堅牢化（[#763](https://github.com/pooza/capsicum/issues/763)〜[#765](https://github.com/pooza/capsicum/issues/765)）を行いました。**
