...
- **v1.30** — macOS push 通知の本配線（capsicum-relay 拡張）、デスクトップ 3 OS 共通の通知経路（Mastodon `user` stream / Misskey `main` channel を WebSocket で受けて OS のローカル通知に流す設計、[#569](https://github.com/pooza/capsicum/issues/569)）、Linux / Windows での動画再生改善、ドラッグ＆ドロップ添付などの UX 改善
- **v1.32** — デスクトップ / パッケージング周りの構造整理（リファクタ集約）
- **v1.33** — プレイヤー横断ナウプレ取得（Linux MPRIS / Windows SMTC / Spotify 連携）

直近の達成: v1.27 で Windows の Microsoft Store 公開（Partner Center 手動 publish ルート、2026-05-20 審査通過）、v1.28 で Misskey のグループチャット対応。
